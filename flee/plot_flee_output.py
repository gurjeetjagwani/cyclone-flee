...
import numpy as np
import pandas as pd
from postprocessing import analysis as a
...
def plot_camps(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    data_filtered = data.drop(
        [
            ...
            "total residents (simulation)",
            "raw UNHCR refugee count",
            "residents in shelters (simulation)",
            "refugee_debt",
        ],
        axis=1,
    )
...
def plot_numagents(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    if "refugee_debt" in data.columns:
        ...
        data.loc[
            :,
            ["total residents (simulation)", "residents in shelters (UNHCR)", "raw UNHCR refugee count"],
        ].plot(linewidth=5)
...
def error_quantification(config, data, name, naieve_model=True):
    ...
    simtot = data["residents in shelters (simulation)"].to_numpy().flatten()
...
def plot_flee_output(input_dir: str, output_dir: str, mscale: bool = False) -> None:
    ...
    if isfile(whole_df_PATH):
        whole_df = pd.read_csv(whole_df_PATH, sep="\s+", encoding="latin1", index_col="Day")
